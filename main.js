...
async function getRandomPoster() {
  ...
  let movieOrPoster = Math.random() > 0.5; // Using Math.random() directly to get 0 or 1
  let home = document.querySelector("#home")
  let element;

  if (movieOrPoster) {
    // Check if the movie exists in the moviesIds array and has a valid backdrop_path
    if (moviesIds[random]) {
      element = await getMovie(moviesIds[random]);
      if (element && element.backdrop_path) {
        home.innerHTML = `
    ...
    // Check if the TV series exists in the tvSeriesIds array and has a valid backdrop_path
    if (tvSeriesIds[random]) {
